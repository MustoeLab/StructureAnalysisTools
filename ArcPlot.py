...
# Version 2.2 (Khoa) with annotation plotting option
# ---------------------------------------------------------------------------------------
...
class ArcPlot(object):
    ...
    def __init__(self, title='', fasta=None):
        ...
        self.annotation = [] # annotation labels
    ...
    def addANNO(self, ANNO, length, panel, colors=None):
        ...
        colordict = {}
        for cl, col in zip(groups, colors):
            colordict[cl] = col
        ...
        while plot_ratio_used < 0.9:
            if len(ANNO['label'].unique()) <= rbp_count:
                break
            else:
                plot_ratio_used += 0.1
                anno_count += 2
        ...
        for idx, row in ANNO.iterrows():
            ...
            clip_path = Path(coordinate, codes)
            clip_patch = patches.PathPatch(clip_path, facecolor=row['color'], \
            ...
            annotation = (label, annotation_x_coor, annotation_y_coor, row['color'], height_per_rbp)
            self.annotation.append(annotation)

            if panel == 1:
                self.topPatches.append(clip_patch)
                self.height[1] = length*sqrt(len(ANNO))*0.02
            else:
                self.botPatches.append(clip_patch)
...
class ANNO():
    """CLIP object to process ENCODE eCLIP file"""
...
def parseArgs():
    ...
    prs.add_argument("--annolist", type=str, default=None, help='An ordered list of labels to plot separated by comma. \
                                                    (e.g. A,B will only plot features with "A" and "B" labels in order.')
